...
import MapaCircuito from './MapaCircuito';


export default function StatsCards({ laps = [], selectedLap, activeSession }) {
    let mejorVuelta = '--:--.---';
    if (laps.length > 0) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            {/* LA CUARTA TARJETA ES EL MAPA */}
            <div className="bg-[#23262A] rounded-lg border border-[#2d3136] overflow-hidden">
                <MapaCircuito
                    datoCircuito={selectedLap?.telemetry_logs?.[0]?.telemetry_json}
                    posicionDirecto={activeSession?.last_status_json?.position}
                />
            </div>
        </div>
    );
